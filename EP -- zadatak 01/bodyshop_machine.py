...
class BodyshopMachine:
    """
    Represents a bodyshop machine that processes orders.
    """    

    def __init__(self, id=2, processing_time=2):
        """
        Initializes a BodyshopMachine object.

        Args:
            id (int): The ID of the bodyshop machine.
            processing_time (int): The time taken by the machine to process an order.
        """
        self.name = 'Bodyshop Machine'
        self.id = id
        self.topic = 'bodyshop'
    ...
    def run(self, client):
        """
        Simulates the processing of an order by the bodyshop machine.
        ...
        # Publish a message to the "bodyshop" topic
        message = f'{formatted_datetime} - {self.message}'
        client.publish(self.topic, message)

machine = BodyshopMachine()

# MQTT broker information
broker_address = "localhost"
broker_port = 1883
topics = ['stamping', 'control']
